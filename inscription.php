<?php
    ...
    if (!empty($_POST['email'])) 
    {
        if (preg_match('#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#',$_POST['email'])) // Test si l'adresse mail est valide
        {
            ...
            $requete = 'SELECT mailUtilisateur FROM Utilisateur WHERE mailUtilisateur = "'.$_POST['email'].'"';
            $resultat = mysqli_query($bdd,$requete); 
            if (mysqli_num_rows($resultat) == 0) // Si elle n'existe pas deja alors
            {
                $cpt++;
            }
            else
   ...
   if ($cpt == 5)
   {
        ...
        $requete = "INSERT INTO `Utilisateur`(`mailUtilisateur`, `mdpUtilisateur`, `nomUtilisateur`, `prenomUtilisateur`, `adresseUtilisateur`, `codePostalUtilsateur`, `villeUtilisateur`, `paysUtilsateur`, `telUtilisateur`, `verificationUtilisateur`) 
